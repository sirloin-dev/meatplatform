...
- youjin: flutter에서 theme 속성(테마 속성)은 앱의 일괄되는 테마를 정할 수 있다. 기본적으로는 `theme: ThemeData(
      ...
      ),` 이라고 쓴다. primarySwatch는 theme 속성 덩어리기 때문에 Colors는 Accent가 붙은 키워드는 사용 할 수 없다. (예시: pinkAccent) 
      Accent가 붙은 키워드를 사용하려면 swatch가 아닌 단일 color로 속성을 지정해야한다. ->  ` theme: ThemeData(
