...
2. `GestureDetector()` -> 여러 세밀한 클릭기능(확대, 축소 drag 등)들을 제공하지만, 잉크가 퍼지듯한 애니메이션효과(리플애니메이션 Ripple Animation)를 제공하지 않고 있다.
    대신, `InkWell()` -> 기본 탭들은 제공하지만. `GestureDetector`보다는 세밀하지 않고, 그 수가 적고 간단하지만, 터치시 효과주는 리플 애니메이션효과를 제공한다.
    
3. 사용자 프로필을 원으로 나타내고 싶을 때에는 https://api.flutter.dev/flutter/material/CircleAvatar-class.html[CircleAvatar class] 를 활용하면 더 깔끔하게 구현가능하다.
