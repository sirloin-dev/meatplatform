...
- gon : spring data mongo를 활용해 mongodb에 다하여 알게된 한주 였습니다. sql과 다르게 Document를 쌓는 방식으로 데이터를 관리하는데 덕분에 데이터를 쌓는데에 있어서는 굉장히 빠르다. 더부렝 데이터를 사용하기위해 정제해야하는데 aggregation method를 이용하여 가공하는 쿼리도 제공한다. 하지만 연관관계가 없어 cascade가 지원이 안된다. 또한 메모리를 더 많이 사용하는것으로 알고있다.
