...
[[section-202209-W3]]
=== 2022년 9월 3주
- hwan: Admin 같은 내부 고객용 app 만들때도 build pipeline, test 등 제품 만들때랑 똑같은 개발 프로세스를 태우는게 좋다. 내부 고객의 불편은 빠르게 수집할 수 있으니 덜 엄격해도 된다고 할 수도 있겠지만... 내부 고객의 생산성을 깎아먹는게 결국은 팀, 조직 전체의 생산성을 깎아 먹기 때문이다.
- gon: 인증되지 않은 라이브러리를 사용할때는 해당 github의 이슈부분을 확인하는것이 중요한것 같다. 임베디드 레디스를 사용할려고 최신버젼을 깔았는데 최신버젼에서 logback 의존문제가있어 해당부분으로 시간을 좀 허비하였다.
- yoonho: 주변을 돌아보게 되었다. + 
상을 치루면서 생각보다 같이 찍었던 사진이 많이 없었다는 것을 깨달았다. + 
일상같이 지나가서 몰랐던 사람들의 감사함과 소중함을 되새기게 되었다
- youjin: 주말에 부모님과 부산 롯데월드에 다녀왔다. 롯데월드 주차요금 정산 방법에는 모바일, 사전정산기, 출구정산기 3가지 방법이 있었는데 파크 이용시, 모바일만 3,000원이고 나머지는 6,000원이다. 모바일 사전정산은 '나이스파크' 라이브러리를 이용하는 듯 했는데 결제 수단에 무조건 카드를 등록하거나 카카오페이나 토스페이로 하더라도 계좌를 다 연결시켜줘야했던 점이 너무 짜증났던 경험이었다. 
- wongue: 연휴간 심심해서 종종 하던 '철권7' 의 리플레이 파일을 텍스트 에디터로 열어봤다. +
시계열 데이터로 이루어져 있을거란 짐작은 맞았는데, 문서의 내부 인코딩이 UTF-8 표준으로는 알아 볼 수 없는 포멧이여서 당황했다.
...
timestamp나 counter등을 사용해서 생성하는것이 최선일 듯 하다.
