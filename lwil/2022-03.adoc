...
- wongue: 서초 1,2 동대 예비군을 다녀오면서 역시 서울이구나 싶었다. +
 장비가 이전에 전해들은것에 비해 상상 이상으로 좋아 많이 놀랐었다.
...
- yoonho: 애플 로그인은 jwt를 클라이언트에 반환한다. +
애플에서 제공하는 공개키 인증서를 통해 서명을 검증하는 것으로 로그인에 이용할 수 있다.
