...
- wongue: 결함육 중에 '근염' 이란, 상품에 정말 염증이 있는 케이스를 의미하기 보단, 근섬유의 괴사 혹은 변성으로 인해 근육이 있어야 할 자리에 대체 결합조직인 지방이 과다하게 축척되어 있는 하자품을 의미한다.link:https://blog.naver.com/zzanggem/220723456942[자료] +
그럼, 사람에게도 이와 같은 일이 발생 할 수 있을까? +
찾아본 결과, 이와 비슷한 현상이 사람에게 발생할 경우 이를 근지방증이라고 부르며 발생시 10년 내 사망률이 무려 15%(%p 가 아니다.) 증가한다고 한다. link:https://m.health.chosun.com/svc/news_view.html?contid=2023051901440[관련 기사] +  
