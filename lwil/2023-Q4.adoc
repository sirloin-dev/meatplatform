...
위 쿼리와 아래 쿼리의 결과는 실제로 다르지 않지만 아래 쿼리는 정렬 조건이 명확하게 들어나지 않고 있고 의도치 않은 버그가 발생할 가능성이 있다.

- gon  :

spring의 LookUp 기능인데 다음과 같은 경우를 해결할 때 사용하면 좋다. 거의 대부분의 애플리케이션에서 bean 전략을 싱글톤으로 사용합니다. 
