...
- gon : 내가 네이버와 통신을 한다 가정하였을때 통신중 Frame 단위의 헤더의 address는 네이버의 mac address인줄 알았으나 게이트웨이의 mac address라고 한다. +
사실 조금만 더 생각해보면 되는데 Frame은 L2계층인데 네이버에 통신할려면 라우팅을위해 L3이상까지 가야하기때문이다.
