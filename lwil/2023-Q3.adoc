...
(
    `users_seq`     BIGINT      UNIQUE,
    `users_id`      BINARY(16)  NOT NULL,
    `new_nickname`  VARCHAR(32) NOT NULL

    CONSTRAINT `uk_users_personal_config_users_seq` FOREIGN KEY (`users_seq`)
        REFERENCES `users` (`seq`)
    CONSTRAINT `fk_users_personal_config_users_id` FOREIGN KEY (`users_id`)
        REFERENCES `users` (`id`)
        ON DELETE CASCADE
)
----
