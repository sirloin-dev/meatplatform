...
val result = divisor.divide(num1, num2)
----
+
...
[source, kotlin]
----
`when`(divisor.divide(any(), any())).thenReturn(...)
----
+
으로 하기 마련인데,
+
...
[source, kotlin]
----
`when`(divisor.divide(any(), any(), any())).thenReturn(...)
----
+
로 두면 method call 이 mock 의 조건과 일치하지 않아 `thenReturn` 의 의도가 제대로 전달되지 않는 문제가 있다. 이런 경우에는:
+
[source, kotlin]
----
`when`(divisor.divide(any(), any(), eq(null))).thenReturn(...)
----
+
로 mock 호출조건을 설정해야 하며 이를 편리하게 하려면 org.mockito.kotlin:mockito-kotlin 라이브러리를 쓰는 편이 좋다.
+
라이브러리 없이 해결하기에는 matcher 를 일일히 구현해 주는 일이 매우 번거롭기 때문이다.
