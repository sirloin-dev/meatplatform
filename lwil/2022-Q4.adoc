...
- yoonho: dirty checking 은 객체에 변경이 발생했을 때 따로 persist 를 해주지 않아도 초기의 객체의 snapshot 과 변경 후의 객체를 비교하여 쓰기 지연 저장소에 변경된 쿼리를 넣어 놓기 때문에  flush 하는 순간 변경된 객체에 대한 쿼리가 나가도록 한다. 
