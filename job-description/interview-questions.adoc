...
질문에 대한 답을 모조리 준비해 오셨다고요? 그렇다면 바로 저희와 함께 즐겁게 일하시면 됩니다.
...
. 어플리케이션의 String 을 데이터베이스에 저장하기 위해 고려해야 할 사항에는 어떤 점이 있을까요?

. 데이터의 양이 많아지면 RDBMS 대신 NoSQL 을 쓰라고 하는데, 그럼 large data set 에는 항상 NoSQL 만 써야 할까요? RBDMS 를 이용해 large data set 을 다루려면 어떻게 해야 할까요?
...
. (JPA 경험자 한정) JPA 의 `@Entity` 란 뭔가요? 도메인 객체와 Entity 는 어떤 차이가 있나요?
...
. 압축 알고리즘을 설계하라는 요구사항이 들어왔다면, 어떻게 하시겠습니까?
...
. Callback function 이 뭔가요(또는 Closure)?
...
. 상속의 이점 중 "코드의 중복을 줄여준다" 는 말이 있습니다. 그런데 코드 중복을 줄이기 위해서 상속을 쓰는 것은 매우 좋지 않은 코딩 방식입니다. 이에 대한 후보자님의 의견을 듣고 싶습니다. 
...
. 우리 서비스가 대 성공해서 이용자가 4000만이 되었다고 가정합니다. 이용자 4천만 돌파 기념으로 선착순으로 접속한 사용자에게 보너스 포인트를 주는 이벤트를 운영하려 합니다. 이벤트 운영은 플랫폼이 제공하는 Push 를 활용할 예정입니다. 이 경우, 어떤 점들을 고려해야 할까요?
...
. 크기를 지정하지 않고 `ArrayList` 를 new 로 생성하면 크기 10의 `ArrayList` 가 생성됩니다. Array 는 크기를 넘길 수 없는데 반해 `ArrayList` 는 꽉 찬 List 에 element 를 추가로 더할 수 있습니다. 그렇다면 10개의 element 를 채워넣은 `ArrayList` 의 11번째 item 을 add 하기위해 어떤 일이 일어나는지 설명해주세요.
...
. Spring 은 `@Transactional` 어노테이션 붙인 메소드를 어떻게 트랜잭션을 처리하나요? 그 내부 구현을 상세하게 설명해 주세요.
...
. Equatable, Hashable 에 대해 설명해 주세요. 두 인터페이스를 모두 구현할 때, 어떤 점을 주의해야 할까요?
